...
export function DeployLogView({
  ...
  // Fires once per run, the moment it leaves RUNNING — the parent uses this
  // to drop the run from `activeRuns`, which is what stops the page-level
  // poll (and this component's own render) once provisioning is done.
  const notifiedRef = useRef(false);
  useEffect(() => {
    if (data && data.status !== 'RUNNING' && !notifiedRef.current) {
...
module.exports = {
  apps: [
    ...
    },
    {
      name: 'jarvis-worker',
      cwd: __dirname + '/..',
      // Same shim problem as jarvis-web above, same fix: node_modules/.bin/tsx
      // is a shell script, and PM2 fork mode hands `script` straight to
      // Node's module loader rather than a shell — it must be tsx's real
      // entry file.
...
Unlike the OTT repo, Jarvis has no `deploy.sh` of its own to write `.env`
for you — create it on the box first:

```bash
cp .env.example .env
# fill in real values: DATABASE_URL, JARVIS_SESSION_SECRET, OTT_REPO_PATH,
# SEED_ADMIN_EMAIL, SEED_ADMIN_PASSWORD
```

Both `next start` (jarvis-web) and `tsx` (jarvis-worker) auto-load `.env`
from their working directory, so nothing else needs to reference it
explicitly — but without this step neither process can start.
