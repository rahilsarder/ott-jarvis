...
**Re-deploying (updating an already-ACTIVE deployment)**

Clicking Deploy again is also how config changes reach a live box, not just code. `deploy.sh` only ever writes a target's `.env` once, during first-time setup — its update path (git pull, install, build, reload) never touches it again. Since every invocation, first-time or update, already carries the deployment's current `DEPLOY_FLUSSONIC_BASE_URL` / `DEPLOY_FLUSSONIC_SECURELINK_KEY` / `DEPLOY_BRAND_NAME`, the update path now `sed`s those three values into the existing `.env` before rebuilding, so an edit made in Jarvis's UI after initial provisioning (a corrected Flussonic URL, a renamed brand) is picked up the next time someone clicks Deploy — no separate "push config" action to remember. This only fires in `--non-interactive` mode: those env vars are Jarvis's own interface to the script and are never set for a human running `ops/deploy.sh` by hand, so an interactive "just pull new code" run can't accidentally blank them. Deliberately excluded from this sync: `SEED_ADMIN_EMAIL` (a one-time seed value, not an ongoing runtime setting — changing it wouldn't rename the already-existing admin) and anything nginx/DNS-related (already covered by the SSH-target-changed reset and the nginx self-heal check above it).
